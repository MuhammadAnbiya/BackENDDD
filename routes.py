...
def login():
    if request.method == 'POST':
        ...
        emailLog = DataUser.query.filter_by(email=email).first()
        passwrodLog = DataUser.query.filter_by(password=password).first()
        
        if emailLog and passwrodLog:
            session['logged_in'] = True
            session['username'] = emailLog.name
